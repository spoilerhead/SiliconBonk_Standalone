...
WorkingSpace::WorkingSpace(Magick::Blob &inputProfile) {
    cmsHPROFILE imgProfile = cmsOpenProfileFromMem(inputProfile.data(), inputProfile.length());
    if(imgProfile == NULL) {
    ...
    fromWorkspace = cmsCreateTransform(workingSpace, TYPE_RGB_FLT, imgProfile, TYPE_RGB_FLT, INTENT_PERCEPTUAL, 0);
